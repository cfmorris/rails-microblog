...
class PasswordResetsTest < ActionDispatch::IntegrationTest
  ...
  test "password resets" do
    ...
    patch password_reset_path(user.reset_token),
          params: {email:user.email,
                   user: { password:              "fubarbaz",
                           password_confirmation: "bazfubar" } }
    assert_select 'div#errors' 
    assert_template 'password_resets/edit'
    # Valid password combination
    patch password_reset_url(user.reset_token),
          params: {email:user.email,
                   user: { password:              "fubarbaz",
                           password_confirmation: "fubarbaz" } }
    assert is_logged_in?
    user.reload.reset_digest.nil?
    assert_not flash.empty?
